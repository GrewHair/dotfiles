...
title_list = [ sp.check_output(f"playerctl -p {player} metadata title", shell=True, text=True).strip() for player in player_list ]

r = Rofi()
choice = r.select('Pick video', title_list)[0]

if choice == -1:
    exit()

chosen_player = player_list[choice]
...
# Stop all the players from the players set, that are playing
for player in playing:
    os.system(f"playerctl -p {player} play-pause")

os.system(f"playerctl -p {chosen_player} play-pause")
os.system(f"echo {chosen_player} > /tmp/recent-player")
exit()
